...
public abstract class Terminal implements EmergencyState, OccupancyLevel, Encodable {
     ...
     * For two terminals to be equal, they must: 1) be the same type (i.e. the same
     * concrete subclass of Terminal) and 2) have the same terminal number.
     *
    ...
    public boolean equals(Object obj) {
        ...
        // convert the obj type from Object to Terminal
        Terminal givenTerminal = (Terminal) obj;
        return this.getClass().getSimpleName().equals(givenTerminal.getClass().getSimpleName())
                && this.terminalNumber == givenTerminal.terminalNumber;
    }
    ...
    @Override
    public int hashCode() {
        // choose prime numbers 3 and 5 for this hashCode method
        return this.getClass().getSimpleName().hashCode() * 3 + this.terminalNumber * 5;
    }
    ...
    public String encode() {
        // a string joiner of the terminal encode
        StringJoiner terminalEncode = new StringJoiner(System.lineSeparator());
        terminalEncode.add(String.format("%s:%d:%b:%d",
                this.getClass().getSimpleName(),
                this.terminalNumber,
                this.emergency,
                this.gates.size()));
        for (Gate gate : this.gates) {
            terminalEncode.add(gate.encode());
        }
        return terminalEncode.toString();
    }
}
